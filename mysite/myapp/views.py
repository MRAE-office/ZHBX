...
def show(request):
    ...
    if request.method == 'POST':  # post方法
        ...
        if request.POST.get('btnFind'):  # 如果是name值为btnFind的查询按钮。has_key方法在python2中是可以使用的，在python3中删除了。
            ...
            print(
                list)  # [('9787121297335', '董伟明', 'Python Web开发实战', '计算机', '2017', 5), ('008', None, 'Python程序设计', '计算机', '2018', 10)]
            ...
            for rowList in list:  # 循环遍历
                ...
                if "%s" % rowList[0] in checkbox_list:  # 获取用户选择的复选框的value值
                    ...
                    # 借阅图书表增加1条借阅记录的SQL语句
                    sql_insert = u""" insert into  [借阅表]([学号],[ISBN],[借阅日期]) VALUES  ('%s','%s','%s')""" % (
                    userID, rowList[0], dt)
...
def Borrow(request):
    ...
    if request.method == 'POST':
        if request.POST.get('btnCanceleOrder'):  # 如果是name值为btnFind的查询按钮。has_key方法在python2中是可以使用的，在python3中删除了。
            ...
            print(request.POST.getlist("checkbox"))  # ['9787121297335', '008']列表
            print(
                listBorrow)  # [('9787121297335', '董伟明', 'Python Web开发实战', '计算机', '2017', 5), ('008', None, 'Python程序设计', '计算机', '2018', 10)]
...
def insertbooks(request):
    if request.method == 'POST':  # post方法
        ...
        PublicationTime = request.POST.get('txtBookDate')
        BookType = request.POST.get('txtBookType')
        ...
        try:
            ...
            sql = u""" insert into  [图书表]([ISBN],[书名],[数量],[作者],[出版时间],[类型]) VALUES  ('%s','%s','%s','%s','%s','%s')""" % (
            ISBN, bookTitle, BookNum, BookAuthor, PublicationTime, BookType)
            print(sql)
